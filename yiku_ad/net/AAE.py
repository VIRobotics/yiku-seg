...
import os
from ..utils.init import init_net
from ..utils.image import compare_images,save_images
class AAE(nn.Module,BaseModel):
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            return x,self.decoder(feature)

    def save_images(self, data):
        images = data['img']
        paths = os.path.join(self.opt.save_dir, self.opt.object)
        paths = os.path.join(paths, "result")
        anomaly_img = compare_images(images, self.generated_imgs, threshold=self.opt.threshold)
        save_images(anomaly_img, paths, data)
